  ...
  return (
    <>
      ...
      <nav className="nav-mobile-bar" style={{ display: 'none', position: 'sticky', top: 0, zIndex: 100, background: '#040404', borderBottom: '0.3px solid rgba(255,255,255,0.06)', height: 60, alignItems: 'center', padding: '0 20px' }}>
        ...
        {/* Explore Now — right */}
        <Link href={buyHref} onClick={() => { if(typeof window !== 'undefined') sessionStorage.setItem('nz_flow','neozap') }} style={{ marginLeft: 'auto', background: 'white', borderRadius: 6, height: 30, padding: '0 14px', display: 'flex', alignItems: 'center', textDecoration: 'none', fontSize: 12, fontWeight: 600, color: 'black', whiteSpace: 'nowrap' }}>
          Explore Now
        </Link>
      </nav>
