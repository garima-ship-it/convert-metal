...
interface Card {
  ...
  imgCol?: string | null; imgChip?: string | null; imgBack?: string | null
  colors?: CardColor[]
}
  ...
  return (
    <div style={{ width: 440, position: 'relative' }}>
      ...
      >
            ...
            <div key={i} style={{ flexShrink: 0, width: 440, height: 300, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', background: SLIDE_GRADIENTS[i] }}>
              {i === 0 && card.imgChip && (
                <>
                  ...
                  {hueColor && <div style={{ position: 'absolute', inset: 0, background: hueColor, mixBlendMode: 'hue' as const, opacity: 0.6 }} />}
                  <div style={{ position: 'absolute', top: '50%', left: 20, transform: 'translateY(-50%)', pointerEvents: 'none' }}>
                    <ChipSVG />
                  </div>
                </>
              )}
              {i === 0 && !card.imgChip && <div style={{ width: 64, height: 64, background: 'rgba(255,255,255,0.12)', borderRadius: '50%' }} />}
              {i > 0 && card.imgCol && (
                <>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
              ...
              {i > 0 && !card.imgCol && <div style={{ width: 64, height: 64, background: 'rgba(255,255,255,0.12)', borderRadius: '50%' }} />}
        ...
        <button onClick={() => go(idx - 1)} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', width: 36, height: 36, borderRadius: '50%', background: 'rgba(0,0,0,0.55)', border: '1px solid rgba(255,255,255,0.18)', color: 'white', fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 5 }}>←</button>
        <button onClick={() => go(idx + 1)} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', width: 36, height: 36, borderRadius: '50%', background: 'rgba(0,0,0,0.55)', border: '1px solid rgba(255,255,255,0.18)', color: 'white', fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 5 }}>→</button>
      </div>
