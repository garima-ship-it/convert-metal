    ...
    <>
      ...
      <div className="product-desktop" style={{ display: 'flex', gap: 60, padding: '0 79px 60px' }}>
        <div style={{ width: 500, flexShrink: 0 }}>
          <div style={{ width: 500, height: 320, borderRadius: 16, overflow: 'hidden', background: 'linear-gradient(135deg,#1a1a2e,#16213e)', marginBottom: 16 }}>
            {mainImg && <img src={mainImg} alt={card.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
          </div>
          {images.length > 1 && (
        ...
        <div style={{ flex: 1 }}>
          {card.bestseller && <div style={{ display: 'inline-block', background: 'rgba(245,158,11,0.15)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 4, padding: '3px 10px', fontSize: 11, fontWeight: 700, color: '#f59e0b', marginBottom: 12 }}>★ Bestseller</div>}
          <h1 style={{ fontSize: 32, fontWeight: 800, marginBottom: 8 }}>{card.name}</h1>
          ...
          {card.colors && card.colors.length > 0 && (
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {card.colors.map((c,i) => (
                  <div key={i} onClick={() => setSelectedColor(i)} title={c.name}
          ...
          {card.features && card.features.length > 0 && (
            <ul style={{ marginBottom: 24, paddingLeft: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 8 }}>
              {card.features.map((f,i) => (
                <li key={i} style={{ display: 'flex', gap: 8, fontSize: 14, color: '#d1d5db' }}>
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8 }}>
              <button onClick={() => setQty(q => Math.max(1,q-1))} style={{ width: 40, height: 50, background: 'none', border: 'none', color: 'white', fontSize: 20, cursor: 'pointer' }}>−</button>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {['🔒 Secure Checkout', '🚚 Free Shipping', '✓ COD Available'].map(b => (
              <div key={b} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 20, padding: '4px 12px', fontSize: 11, color: '#9ca3af' }}>{b}</div>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="product-mobile" style={{ display: 'none', paddingBottom: 100 }}>
        ...
        <div style={{ padding: '16px 20px 0' }}>
          {card.bestseller && <div style={{ display: 'inline-block', background: 'rgba(245,158,11,0.15)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 4, padding: '3px 10px', fontSize: 11, fontWeight: 700, color: '#f59e0b', marginBottom: 10 }}>★ Bestseller</div>}
          <h1 style={{ fontSize: 22, fontWeight: 800, marginBottom: 8 }}>{card.name}</h1>
          ...
          {card.colors && card.colors.length > 0 && (
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                {card.colors.map((c,i) => (
                  <div key={i} onClick={() => setSelectedColor(i)} title={c.name}
                    style={{ width: 30, height: 30, borderRadius: '50%', background: c.hex, cursor: 'pointer', border: selectedColor===i?'3px solid white':'2px solid rgba(255,255,255,0.2)', boxSizing: 'border-box' }} />
                ))}
              </div>
            </div>
          )}
        ...
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#0a0a0a', borderTop: '1px solid rgba(255,255,255,0.08)', padding: '12px 20px', display: 'flex', gap: 12, alignItems: 'center', zIndex: 50 }}>
          <div style={{ display: 'flex', alignItems: 'center', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8, flexShrink: 0 }}>
            ...
            <span style={{ width: 30, textAlign: 'center', fontSize: 15 }}>{qty}</span>
            <button onClick={() => setQty(q => q+1)} style={{ width: 36, height: 44, background: 'none', border: 'none', color: 'white', fontSize: 20, cursor: 'pointer' }}>+</button>
          </div>
          <div style={{ flex: 1 }}>
