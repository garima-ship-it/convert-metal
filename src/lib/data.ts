...
export async function fetchCatalog(): Promise<CatalogData | null> {
  try {
    ...
    const url = `https://raw.githubusercontent.com/${owner}/${repo}/main/neozap-data.json?t=${Date.now()}`
...
export const FAQ_ITEMS = [
  { q: 'Is the DIY process difficult to do?',         a: 'Not at all! Most customers swap their chip in under 5 minutes.' },
  { q: 'Will my card still work after the transfer?', a: 'Yes! Same chip, same PIN, same contactless payments.' },
